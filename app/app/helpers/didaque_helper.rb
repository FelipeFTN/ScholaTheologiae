module DidaqueHelper
  include ApplicationHelper

  def didaque_get_parts
    begin
      uri = URI("#{BASE_URL}/books/didaque")
  ...
  def didaque_get_chapter(part, chapter)
    begin
      # Encode the part to ensure it is ASCII only
      encoded_part = URI.encode_www_form_component(part)
      uri = URI("#{BASE_URL}/books/didaque/#{encoded_part}/#{chapter}")
      response = Net::HTTP.get(uri)
      # Force encoding to UTF-8 to avoid encoding issues
      response = response.force_encoding('UTF-8')
